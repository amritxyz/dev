...
\usepackage{listings}                % ← fixed: was "listing"
\usepackage{chngcntr}
\usepackage[numbers,compress]{natbib}
\usepackage{booktabs}   % for \toprule, \midrule, \bottomrule
\usepackage{array}      % already loaded by most templates, but safe to add

% ----------------- FIGURE NUMBERING: Figure 1.1, 1.2, 3.1 etc. -----------------
\counterwithin{figure}{chapter}
\counterwithin{table}{chapter}

% ----------------- CAPTION STYLE: "Figure 1.1 Use Case Diagram..." (exactly what you want) -----------------
\usepackage{caption}
\captionsetup[figure]{
  labelfont=bf,
  textfont=normalfont,
  justification=centering,
  labelsep=space,      % ← THIS IS THE KEY: only one space, no colon, no period
  skip=12pt,
  name=Figure          % ensures it says "Figure" not "Fig."
}
\captionsetup[table]{
  labelfont=bf,
  textfont=normalfont,
...
% Chapter style (your original – perfect)
\titleformat{\chapter}[hang]
  {\bfseries\fontsize{16pt}{18pt}\selectfont\centering}
  {CHAPTER \thechapter:}{5pt}{\MakeUppercase}
...
\begin{center}
    % --- Logo ---
    \includegraphics[width=2.5cm]{logo.png}\\[0.8cm]  % reduced from 3cm

    % --- University & Faculty ---
    \large\textbf{Tribhuvan University}\\[0.15cm]
    \large\textbf{Faculty of Humanities and Social Sciences}\\[0.8cm]

    % --- Project Title ---
    \Large\textbf{Personal Expense Tracker}\\[0.4cm]
    \textbf{A PROJECT REPORT}\\[0.8cm]

    % --- Submitted to ---
    \large\textbf{Submitted to}\\[0.1cm]
    \large\textbf{Department of Computer Application}\\[0.1cm]
    \large\textbf{Butwal Kalika Campus}\\[0.8cm]

    \normalsize
    \textbf{\textit{In partial fulfillment of the requirements for the}}\\[0.15cm]
    \textbf{\textit{Bachelor’s in Computer Application}}\\[1.0cm]

    % --- Submitted by ---
    \large{Submitted by:}\\[0.1cm]
    \normalsize
    Susma Tandan [6-2-1055-199-2023]\\[0.1cm]
    Amrit Bhattarai [6-2-1055-139-2023]\\[1.0cm]

    \large{Under the Supervision of}\\[0.1cm]
    \textbf{Nirmal Aryal}\\[1.0cm]
...
The Personal Expense Tracker is a web-based application developed to simplify personal financial management by enabling users to record, categorize, and analyze income and expenses through an intuitive interface. Built with React for the frontend, Node.js and Express for the backend, and SQLite3 for data storage, the system supports user authentication, CRUD operations on transactions, and visualizations like doughnut charts and bar graphs to reveal spending patterns and support budgeting decisions.

Key features include secure login via JWT, category-based transaction tracking, real-time dashboards, and reports filtered by date or category, addressing challenges like manual record-keeping errors and lack of insights from notebooks or spreadsheets.

The project demonstrates technical feasibility with a modular MVC architecture, ensuring usability, performance under 3 seconds per action, and cross-browser compatibility, while limitations such as manual entry and no bank integration highlight areas for future enhancements like API syncing and mobile apps.

This tool promotes financial literacy by providing actionable insights, helping users avoid overspending, set goals, and achieve stability across personal and small-scale business use.
...
We would like to sincerely thank everyone who played a role in the successful completion of our project, \textbf{Personal Expense Tracker}.

First and foremost, we express our heartfelt gratitude to our supervisor, \textbf{Mr. Nirmal Aryal}, for his invaluable guidance, constructive feedback, and continuous encouragement throughout the project. His expertise and insights have been instrumental in helping us shape and complete this work successfully.

We are also deeply grateful to \textbf{Butwal Kalika Campus} and the \textbf{Department of Computer Applications} for providing us with the resources, opportunities, and a supportive environment that allowed us to turn our ideas into reality.

Finally, we would like to recognize and appreciate the dedication and collaboration of our project team. The teamwork, perseverance, and shared efforts were crucial in bringing this project to completion.
...
% Table of Contents title in 16pt bold
\centering\renewcommand{\contentsname}{\bfseries\fontsize{16pt}{18pt}\selectfont Table of Contents}
...
\tableofcontents
\newpage
% === ADD THESE LINES JUST AFTER \tableofcontents AND BEFORE \pagenumbering{arabic} ===


% Make List of Figures and List of Tables titles also 16pt bold
\renewcommand{\listfigurename}{\centering\bfseries\fontsize{16pt}{18pt}\selectfont List of Figures}
\renewcommand{\listtablename}{\centering\bfseries\fontsize{16pt}{18pt}\selectfont List of Tables}
...
\toprule[1.5pt]  % thick top line
\textbf{Abbreviation} & \textbf{Full Form} \\
\midrule[1pt]    % medium line after header
...
UI   & User Interface \\
UX   & User Experience \\
\bottomrule[1.5pt]  % thick bottom line
\end{tabular}
\newpage
...
Many researchers and developers have studied ways to help people manage their personal finances more effectively. Traditional methods, such as writing expenses in a notebook or using basic spreadsheets, are often time-consuming, prone to mistakes, and do not provide clear insights into spending habits \cite{manchanda2012expense}. To address these issues, several digital expense tracker systems have been developed.

Mobile applications like Mint, YNAB (You Need a Budget), and PocketGuard allow users to categorize transactions, track income and expenses, and visualize data through charts and graphs \cite{chandini2019online}. Some studies show that using these tools helps individuals save more money, understand spending patterns, and plan budgets efficiently. Other research focuses on building simple web-based systems that provide CRUD functionality for income and expenses, making it easier for users to maintain accurate records \cite{verma2024research}.
...
\subsubsection{Use Case Modeling (Use Case Diagram)}
The Use Case Diagram (Figure~\ref{fig:usecase}) captures the functional requirements of the Personal Finance Management System from the user’s perspective. It illustrates the interactions between the primary actor (User) and the system.

\vspace{20pt}

\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.6]{use_case_diagram.png} % Replace with your use case diagram
...
The Entity-Relationship (ER) Diagram (Figure~\ref{fig:er_diagram}) represents the data entities and their relationships. The main entities are \textbf{User}, \textbf{Income}, and \textbf{Expense}. A User can have many Transactions, and each Transaction belongs to one Category.
...
The design of the interface focuses on being clear, easy to navigate, and making key financial insights immediately accessible. It’s built to create a welcoming experience, so managing your money feels simple and not overwhelming.

The main screens are designed to provide everything you need through clean layouts and intuitive interactions, making it easy to find what you're looking for and take action quickly:

\begin{itemize}
    \item \textbf{Dashboard}: This is your financial overview at a glance. It shows you key details like total income, expenses, and your current balance. You’ll also see interactive charts that give you a quick sense of your spending trends.
    
    \item \textbf{Income}: Here, you can view your income over time with easy-to-read line and bar charts. The \texttt{Add Income} button makes it simple to add new earnings, and you can filter the chart to show data for specific time periods—weekly, monthly, or yearly.
    
    \item \textbf{Expenses}: This section helps you visualize your spending. It includes doughnut charts that break down your expenses by category, along with a list of recent expenses and an \texttt{Add Expense} button for quick updates. You can also filter the charts based on different time periods to track spending trends.
\end{itemize}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.6]{physical_dfd.png}
...
\begin{itemize}
    ...
    Manages user registration and login via \textbf{POST /register} and \textbf{POST /login} endpoints. Passwords are currently stored in plain text. Upon successful login, a JSON Web Token (JWT) is generated using \textbf{jwt.sign()} with a secret key and 100-hour expiry. This token must be included in the \textbf{Authorization: Bearer token} header for all protected routes.
    ...
    \begin{itemize}
        ...
        \item \textbf{PUT /profile}: Updates name, email, password and avatar.
    \end{itemize}
...
app.post('/expense', authenticateJWT, (req, res) => {
  ...
  if (!amount || !categories || !subcategories || !date) {
    return res.status(400).json({ message: "Categories, Amount and Date
      are required" });
  }
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.23]{dashboard.png}
    \caption{Dashboard of Personal Expense Tracker}
    \label{fig:dashboard}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Profile Section of Personal Expense Tracker}
    \label{fig:profile}
\end{figure}
\newpage
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.23]{income_page.png}
    \caption{Income Page of Personal Expense Tracker}
    \label{fig:Income page}
\end{figure}


\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.23]{add_income_modal.png}
    \caption{Form / Modal to Add Income}
    \label{fig:Add income}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.23]{expense_page.png}
    \caption{Expense Page of Personal Expense Tracker}
    \label{fig:Expense page}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.23]{add_expense_modal.png}
    \caption{Form / Modal to Add Expense}
    \label{fig:Expense page}
\end{figure}
